...
void SegmentTree::Build_Tree(int top, int l, int r)
{
    ...
    if (l == r)
    {
        v.at(top).max = v.at(top).l = a[l];
        return;
    }
    int mid = (l + r) >> 1;
    Build_Tree(2 * top, l, mid);
    Build_Tree(2 * top + 1, mid + 1, r);
    v.at(top).max = max(v.at(top * 2).max, v.at(top * 2 + 1).max);
    v.at(top).min = min(v.at(top * 2).min, v.at(top * 2 + 1).min);
}
...
void SegmentTree::Pushdown(int top)
{
    ...
    v.at(top * 2).max += v.at(top).lazy;
    v.at(top * 2).min += v.at(top).lazy;
    v.at(top * 2 + 1).max += v.at(top).lazy;
    v.at(top * 2 + 1).min += v.at(top).lazy;
...
void SegmentTree::update(int top, int l, int r, ll k)
{
    if (v.at(top).l == l && v.at(top).r == r)
    {
        v.at(top).max += v.at(top).lazy;
        v.at(top).min += v.at(top).lazy;
        v.at(top).lazy += k;
        return;
    }
    Pushdown(top);
